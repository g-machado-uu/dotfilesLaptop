...
// The month view that drops out of the clock in the status bar. This is only
// the content: the card, its border and the drop animation come from the
// BarPanel that hosts it.
Item {
    id: root
    ...
    // The old window re-checked the date each time it was opened. The panel is
    // long-lived now, so it watches the clock instead: when the day rolls over
    // the highlight follows it without anyone having to reopen the calendar.
    Timer {
        interval: 60000
        running: true
        repeat: true
        onTriggered: {
            let now = new Date()
            if (now.getDate() === root.todayDate && now.getMonth() === root.todayMonth)
                return
            let wasOnToday = root.currentMonth === root.todayMonth
                && root.currentYear === root.todayYear
            root.todayDate = now.getDate()
