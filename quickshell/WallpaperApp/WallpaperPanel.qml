...
// The wallpaper picker, dropping out of the status bar. Content only: the
// silhouette, translucency and drop animation come from the BarPanel hosting
// it. It used to be a full-height panel pinned to the left screen edge.
Item {
    id: root

    // Natural width of the panel body. The height is set by the bar, which is
    // the only thing that knows how much screen there is to fill.
    readonly property real panelWidth: 420

    // Mirrors the hosting panel's state; anything that wants to dismiss the
    // panel calls close() rather than writing to it.
